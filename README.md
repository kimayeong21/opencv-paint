...
- 코덱(Codec) 설정이 필요합니다. (Windows: `DIVX`, Mac/Linux: `XVID` or `MJPG`)
- **주의:** `frameSize`가 실제 받아오는 영상 크기와 다르면 저장되지 않습니다.
...
## 실습 과제 : **스노우(SNOW) 앱 필터 구현**

웹캠 영상에서 내 얼굴을 실시간으로 인식하고, 눈이나 코 위치에 재미있는 이미지(안경, 고양이 코, 수염 등)를 합성하는 프로그램을 만듭니다.

1. **준비물:** 투명 배경이 있는 PNG 스티커 이미지 + 웹캠
2. **얼굴 검출:** `CascadeClassifier`를 이용해 매 프레임마다 얼굴 영역(ROI)을 찾습니다.
3. **위치 계산:**
    - 검출된 얼굴 박스 `(x, y, w, h)`를 기준으로 눈/코의 대략적인 위치를 추정합니다.
    - 예: 눈은 얼굴 상단 1/3 지점, 코는 중앙.
4. **이미지 합성:**
    - 스티커 이미지를 얼굴 크기에 맞게 리사이즈합니다.
    - 스티커의 투명 채널(Alpha channel)을 마스크로 활용하여 얼굴 위에 자연스럽게 덮어씌웁니다.
